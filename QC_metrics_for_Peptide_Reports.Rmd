...
packages <- c("data.table", "dplyr", "ggplot2", "stringr", "knitr", "markdown", "tidyr", "reshape2")
if (length(setdiff(packages, rownames(installed.packages()))) > 0) {
    install.packages(setdiff(packages, rownames(installed.packages())))  }
 
library("data.table"); library("dplyr"); library("ggplot2"); library("stringr"); 
library("knitr"); library("markdown"); library("tidyr"); library("reshape2")

```

```{r, include=FALSE}
#Upload files using getwd
pride_cluster_peptides_ALL_version1  <- read.delim(file.path(getwd(),"pride_cluster_peptides_Mouse.tsv"), comment.char="#")
pride_cluster_peptides_ALL_version2 <- read.delim(file.path(getwd(),"pride_cluster_peptides_Mouse.tsv"), comment.char="#")
...
if (nrow(df_final[is.na(df_final$PEH.1),])==0) {
    cat("The new release has not gained new peptides\n")
} else {
...
if (nrow(df_final[is.na(df_final$PEH.2),])==0) {
...
if (sum(df_final_project2$change == 0)) {
    ...
    cat("The number of projects are different\n")
    kable(df_final_project3)
}
...
if (sum(df_final_clusters2$change == 0)) {
    ...
    cat("The number of clusters are different\n")
    kable(df_final_clusters3)
}
